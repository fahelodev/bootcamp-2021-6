Feature: Carrito de Compra
  ...
  Background: Agregar producto
    ...
    Then se debe presionar en boton continuar
    And  ir al carrito de compras

  Scenario: CA-01 - Límite de Stock de un producto
    Given  Dado que el cliente está el en carrito de compras con 1 producto
    When   cliente aumenta la cantidad a pedir del producto haciendo click en el botón "+" del producto a un número mayor de [limite stock]
    Then   Entonces el botón "+" se deshabilita

  Scenario: CA-02 - Total con la máxima garantía Daño Accidental - 3 años
    Given el cliente está el en carrito de compras con al menos 1 producto SIN Garantía Extendida
    When  el cliente agrega la garantia de daño accidental
    Then  Entonces el total a pagar incluye el monto a pagar del producto + el valor de la garantía del producto


  Scenario: CA-03 -  Código promocional inválido
    Given Dado que el cliente está en el carrito con varios productos a comprar
    When  el cliente introduce un número de cupón inválido
